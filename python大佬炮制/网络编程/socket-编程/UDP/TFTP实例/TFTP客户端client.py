...
cmd_buf = struct.pack("!H5sb5sb".encode("UTF-8"), 1, "1.png".encode("UTF-8"), 0, "octet".encode("UTF-8"), 0)

udpSocket.sendto(cmd_buf, ('192.168.22.190', 69))
...
while True:
    ...
    if cmd == 3:  # 是否为数据包
        ...
        # 如果是第一次接收到数据，那么就创建文件
        if currentPackNum == 1:
            recvFile = open("1.png", "wb")  # 以二进行接收数据
